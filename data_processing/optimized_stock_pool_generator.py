...
class OptimizedStockPoolGenerator:
    ...
    def get_limit_up_stocks_from_daily_data(self, date_str: str) -> List[str]:
        ...
        def check_stock_limit_up(df, stock_code):
            # 过滤掉NaT值
            df_clean = df.dropna(subset=['date'])
            # 查找指定日期的数据
            day_data = df_clean[df_clean['date'].dt.date == date_obj]
            if day_data.empty:
                return None
            ...
            # 获取前一个交易日的数据来计算涨停价
            prev_day_data = df_clean[df_clean['date'].dt.date < date_obj]
            ...
            if pd.isna(current_pct_change) or current_pct_change == 0:
                # 如果pct_change是NaN或0，可能是数据问题，尝试其他方式
            ...
            # 根据股票代码判断涨停板比例
            # 对于30开头的创业板股票，使用不同的涨停标准
            if stock_code.startswith('30'):  # 创业板股票
                limit_threshold = 19.5  # 30开头的股票可能适用20%涨停板，使用19.5%作为阈值
            else:
                limit_threshold = 9.75  # 主板及其他板块使用9.75%阈值
            
            if current_pct_change >= limit_threshold:  # 当日涨幅大于等于阈值
                return stock_code
            else:
                # 备用判断：收盘价接近涨停价
                # 计算涨停价（普通股票10%，ST股票5%，创业板20%）
        ...
        print(f"正在处理 {len(all_data)} 个股票数据...")
        start_time = time.time()
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for stock_code, df in all_data.items():
                future = executor.submit(check_stock_limit_up, df, stock_code)
                futures.append(future)
            
            results = [future.result() for future in futures if future.result() is not None]
        
        end_time = time.time()
        print(f"涨停股票识别完成，耗时: {end_time - start_time:.2f}秒")
    ...
    def get_ever_limit_up_not_closed_stocks_from_daily_data(self, date_str: str) -> List[str]:
        ...
        def check_stock_limit_up_not_closed(df, stock_code):
            # 过滤掉NaT值
            df_clean = df.dropna(subset=['date'])
            # 查找指定日期的数据
            day_data = df_clean[df_clean['date'].dt.date == date_obj]
            ...
            # 判断是否曾涨停未封板
            # 方法1：涨幅达到阈值但收盘价未达到涨停价
            # 方法2：最高价触及涨停价但收盘价未达到涨停价
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            futures = []
            for stock_code, df in all_data.items():
                future = executor.submit(check_stock_limit_up_not_closed, df, stock_code)
                futures.append(future)
            
            results = [future.result() for future in futures if future.result() is not None]
        
        end_time = time.time()
        print(f"曾涨停未封板股票识别完成，耗时: {end_time - start_time:.2f}秒")
    ...
    def batch_generate_stock_pool(self, start_date: str, end_date: str, max_workers: int = 20):
        """批量生成指定时间段的股票池数据"""
        from datetime import datetime, timedelta
...
def main():
    ...
    generator = OptimizedStockPoolGenerator()
