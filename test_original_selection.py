...
def test_original_selection():
    ...
    if not os.path.exists(pool_file_path):
        print(f"警告: 未找到{test_date}的pool数据文件，尝试使用最近的日期")
        # 查找最近的pool数据文件
        import glob
        pool_files = glob.glob(os.path.join(project_root, 'full_stock_data', 'pool_data', 'pool_*.json'))
        if pool_files:
            # 获取最近的文件
            pool_files.sort(key=os.path.getmtime, reverse=True)
            pool_file_path = pool_files[0]
            test_date = os.path.basename(pool_file_path)[5:-5]  # 提取日期部分 'pool_YYYY-MM-DD.json'
            print(f"使用最近的pool数据文件: {test_date}")
        else:
            print(f"错误: 未找到任何pool数据文件")
            return
    ...
    # 导入原始版本
    try:
        import select_2026_01_12
        selector = select_2026_01_12.TodayStockSelector()
        ...
        print(f"\n原始版本测试完成，耗时: {execution_time:.2f}秒")
